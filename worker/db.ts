...
export async function dashboard(env:Env){
  ...
  const volunteers=await env.DB.prepare(`
    WITH cm AS (
      SELECT LOWER(email) email,
             COUNT(DISTINCT program) program_count,
             GROUP_CONCAT(DISTINCT program) programs,
             MIN(program) single_program
      FROM contact_mappings GROUP BY LOWER(email)
    )
    ...
    GROUP BY LOWER(COALESCE(v.volunteer_email,v.volunteer_name))
    ORDER BY knownHours DESC,name LIMIT 200`).all<any>();

  const programs=await env.DB.prepare(`
    WITH distinct_cm AS (
      SELECT DISTINCT LOWER(email) email, program FROM contact_mappings
    ),
    counts AS (
      SELECT email, COUNT(*) program_count FROM distinct_cm GROUP BY email
    ),
    vh AS (
      SELECT LOWER(volunteer_email) email,
             SUM(CASE WHEN hours_known=1 THEN hours*quantity ELSE 0 END) known_hours,
             SUM(CASE WHEN hours_known=0 THEN quantity ELSE 0 END) tbd_assignments
      FROM volunteer_slots
      WHERE status='filled' AND volunteer_email IS NOT NULL
      GROUP BY LOWER(volunteer_email)
    )
    ...
    FROM distinct_cm d
    JOIN counts ON counts.email=d.email
    JOIN vh ON vh.email=d.email
    GROUP BY d.program
    ORDER BY knownHours DESC,name`).all<any>();

  const match=await env.DB.prepare(`
    WITH cm AS (SELECT LOWER(email) email,COUNT(DISTINCT program) pc FROM contact_mappings GROUP BY LOWER(email)),
         v AS (SELECT DISTINCT LOWER(volunteer_email) email FROM volunteer_slots WHERE status='filled' AND volunteer_email IS NOT NULL)
    SELECT COUNT(*) total,
           COALESCE(SUM(CASE WHEN cm.pc>=1 THEN 1 ELSE 0 END),0) matched,
           COALESCE(SUM(CASE WHEN cm.pc>1 THEN 1 ELSE 0 END),0) ambiguous,
           COALESCE(SUM(CASE WHEN cm.pc IS NULL THEN 1 ELSE 0 END),0) unmatched
    FROM v LEFT JOIN cm ON cm.email=v.email`).first<any>();
  ...
  const unmatched=await env.DB.prepare(`
    ...
    SELECT COALESCE(NULLIF(v.volunteer_name,''),v.volunteer_email,'Unknown') name,v.volunteer_email email,
           COALESCE(cm.programs,'') programs,COALESCE(cm.pc,0) programCount,
           SUM(CASE WHEN v.hours_known=1 THEN v.hours*v.quantity ELSE 0 END) knownHours,
           SUM(CASE WHEN v.hours_known=0 THEN v.quantity ELSE 0 END) tbdAssignments
    FROM volunteer_slots v LEFT JOIN cm ON LOWER(v.volunteer_email)=cm.email
    WHERE v.status='filled' AND cm.pc IS NULL
    GROUP BY LOWER(COALESCE(v.volunteer_email,v.volunteer_name))
    ORDER BY knownHours DESC,name LIMIT 100`).all<any>();
  return {settings:cfg,summary:{hoursNeeded:needed,hoursFilled:filled,hoursRemaining:Math.max(0,needed-filled),knownHoursNeeded:knownNeeded,knownHoursFilled:knownFilled,estimatedHoursNeeded:Number(s?.tbdNeededQty||0)*est,estimatedHoursFilled:Number(s?.tbdFilledQty||0)*est,fillRate:needed?filled/needed*100:0,activeEvents:e.results.length,totalAssignments:Number(s?.totalAssignments||0),openSlots:Number(s?.openSlots||0),assignedSlots:Number(s?.assignedSlots||0),tbdAssignments:Number(s?.tbdAssignments||0),lastSyncAt:l?.syncTime||null,lastSyncStatus:l?.status||null,contactSyncAt:cl?.syncTime||null,contactSyncStatus:cl?.status||null,contactMappings:Number(contacts?.mappings||0),uniqueContactEmails:Number(contacts?.uniqueEmails||0),matchedVolunteers:Number(match?.matched||0),ambiguousVolunteers:Number(match?.ambiguous||0),unmatchedVolunteers:Number(match?.unmatched||0),volunteerEmails:Number(match?.total||0)},programs:(programs.results||[]).map((x:any)=>({...x,knownHours:Number(x.knownHours||0),tbdAssignments:Number(x.tbdAssignments||0),volunteers:Number(x.volunteers||0)})),events:e.results.map((x:any)=>({...x,hoursNeeded:Number(x.knownNeeded||0)+Number(x.tbdQty||0)*est,hoursFilled:Number(x.knownFilled||0)+Number(x.tbdFilledQty||0)*est})),volunteers:volunteers.results,unmatched:unmatched.results};
}
